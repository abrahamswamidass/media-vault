...
function renderDetails(item) {
  ...
  const rows = [
    ...
    ["Camera", camera || null],
    ["Aperture", item.aperture ? `f/${item.aperture}` : null],
    ["Shutter speed", item.shutter_speed],
    ["ISO", item.iso],
    ["Exposure comp.", item.exposure_compensation],
    ["Focal length", item.focal_length],
    ["35mm equiv.", item.focal_length_35mm],
    ["Metering", item.metering_mode],
    ["Flash", item.flash],
    ["Source", item.source],
    ["Type", item.mime],
